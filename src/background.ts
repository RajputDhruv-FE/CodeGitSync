...
async function connectGitHub(): Promise<GitHubAuthResponse> {
  ...
  console.log(
    "↩️ GitHub authorization completed"
  );
...
async function getGitHubUser(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to get GitHub user: ${response.status} ${error}`
    );
  }
...
async function getRepository(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to check repository: ${response.status} ${error}`
    );
  }
...
async function createRepository(
  ...
  console.log(
    `📦 Repository "${repoName}" does not exist. Creating it...`
  );
    ...
    await githubRequest(
      ...
      "/user/repos",
      {
        ...
        body: JSON.stringify({
          ...
          description:
            "LeetCode solutions synced automatically",
  ...
  console.log(
    `✅ Repository created: ${repository.full_name}`
  );
...
async function getFileSha(
  ...
  const response =
    await githubRequest(
      token,
      `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repoName)}/contents/${encodedPath}`
    );
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to check file ${path}: ${response.status} ${error}`
    );
  }
...
async function pushFile(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to push ${file.path}: ${response.status} ${error}`
    );
  }
...
async function pushSolution(
  ...
  if (!repository) {
    ...
    console.log(
      `📦 Repository already exists: ${repository.full_name}`
    );
  }
  ...
  console.log(
    "🎉 All solution files pushed successfully"
  );
